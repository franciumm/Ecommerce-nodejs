...
if(userif && userif.confirmEmail == true ){return res.redirect(`${req.protocol}://${req.headers.host}/user/login`)}
tokenDec.user.password = bycrypt.hashSync(tokenDec.user.password, parseInt(process.env.HASH_ROUNDS));

tokenDec.user.phone = CryptoJS.AES.encrypt(tokenDec.user.phone.toString(), process.env.PHONE_SIG).toString()
const usercreated =await  UserModel.create(tokenDec.user);


return user ? res.redirect(`${req.protocol}://${req.headers.host}/user/logIn`)
: res.send(`<a href="${req.protocol}://${req.headers.host}/user/signup">Ops looks like u don't have account yet follow me to signup now. </a>`)
})
...
export const newConfirmEmail = asyncHandler(async (req,res,next)=>{
    ...
    if (!user) {return res.send(`<a href="${req.protocol}://${req.headers.host}/user/signup">Ops looks like u don't have account yet follow me to signup now. </a>`)}
